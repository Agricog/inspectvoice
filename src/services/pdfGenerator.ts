 ...
 *   4. Asset inspection results — per-asset condition, defects, notes
...
function renderCoverPage(
  ...
  const infoItems: Array<[string, string]> = [
    ...
    ['Temperature', inspection.temperature_c !== null ? `${inspection.temperature_c}°C` : '—'],
  ];
...
function renderDefectBlock(
  ...
  // BS EN reference + action + timeframe + cost
...
function renderDefectRegister(
  ...
  // Table header
  const colWidths = [55, 50, 190, 80, 60, 60];
